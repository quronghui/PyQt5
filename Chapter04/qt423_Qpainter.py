...
class MainWindow(QMainWindow):  
	def __init__(self,parent=None):  
       ...
       # 创建一个放置图像的QLabel对象imageLabel，并将该QLabel对象设置为中心窗体。 
		...
		self.setCentralWidget(self.imageLabel)  # 设置为中心
       ...
       # 在imageLabel对象中放置图像
		if self.image.load("./images/screen.png"):  	# 加了一个判断
			self.imageLabel.setPixmap(QPixmap.fromImage(self.image))  	# setPixmap()用于设置图片
	...
	def createActions(self):  
		...
		self.PrintAction.triggered.connect(self.slotPrint)   # 打印的槽函数
	
	# 创建一个菜单栏，为此菜单栏添加一个打印的动作；
